...
An Atlassian MCP server with Confluence read+write access must be configured
in Cursor. The skill calls Confluence tools via `CallMcpTool`. Determine the
correct MCP server name by listing available MCP servers — common names
include `user-mcp-atlassian`, `atlassian`, or `confluence`. Store the resolved
server name as `confluence_mcp_server` in the config file so subsequent runs
skip discovery.

## Configuration

Configuration is stored in `~/.cursor/weekly-update.json` (falls back to `~/.claude/weekly-update.json` for cross-tool compatibility). This file is created automatically on first run.

- **Config file**: `~/.cursor/weekly-update.json`
...
Read the config file at `~/.cursor/weekly-update.json` (fall back to `~/.claude/weekly-update.json`).

```bash
cat ~/.cursor/weekly-update.json 2>/dev/null || cat ~/.claude/weekly-update.json 2>/dev/null
```

**If the file exists and contains valid JSON with `parent_page_id`, `space_key`, and `confluence_mcp_server`:** Use those values. Proceed to Step 2.
...
5. **Validate** by calling `getConfluencePage` (via `CallMcpTool` on the discovered MCP server) with the resolved page ID. If it fails, report the error and ask the user to try again. If it succeeds, extract the space key from the response metadata.
...
7. **Save** the configuration:
   ```json
   {
     ...
     "team_name": "<team_name>",
     "confluence_mcp_server": "<server>",
     "team_members": {}
   }
   ```
   Write this to `~/.cursor/weekly-update.json`.
8. **Populate the landing page.** Read the current parent page content. If it looks like a placeholder or has not been set up yet (e.g., no `## How It Works` section), write the landing page template:
   ...
   - Team members add updates anytime during the week using the `/weekly-update` skill
   ...
   Call `updateConfluencePage` (via `CallMcpTool`) with the page ID, keeping the existing title. Use Atlassian Document Format (ADF) or wiki markup as the content format, and set the version comment to "Landing page setup via /weekly-update".
...
9. Confirm: "Configuration saved. Weekly update pages will be created under: [page title] (space: [space_key], team: [team_name]). You can reconfigure anytime by deleting ~/.cursor/weekly-update.json and running the skill again."
...
$saturday = $now.AddDays(-$days_back).ToString("yyyy-MM-dd")
...
1. Check the `team_members` map in the config file (a cached dictionary of `"Full Name": "kerberos_id"` pairs).
...
Use the `confluence_mcp_server` from config for all Confluence `CallMcpTool` calls.

**7a. Check for existing page:**

Call `getConfluencePageDescendants` (via `CallMcpTool`) with the `parent_page_id` from the config file. Request a limited number of results.

Search the returned descendants for a page whose title matches the computed week title from Step 2.

**7b. If the page exists:**

Call `getConfluencePage` (via `CallMcpTool`) with the found page's ID. Request the content in a format suitable for editing (storage format or body content).

Store the page content and page ID for updating in Step 8.

**7c. If the page does NOT exist:**

Create it using `createConfluencePage` (via `CallMcpTool`) with:
...
1. Read the landing (parent) page content:
   Call `getConfluencePage` (via `CallMcpTool`) with the `parent_page_id` from config.
...
5. Update the landing page:
   Call `updateConfluencePage` (via `CallMcpTool`) with the `parent_page_id`, keeping the title `"Weekly Updates"` unchanged. Pass the updated content and set the version comment to `"Added link for week of {saturday} to {friday}"`.
...
Call `updateConfluencePage` (via `CallMcpTool`) with the existing page ID, keeping the title unchanged. Pass the updated content with the new entry inserted and set the version comment to `"Added [section > category] entry via /weekly-update"`.
